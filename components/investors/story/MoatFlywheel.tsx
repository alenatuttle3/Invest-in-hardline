...
/* Scroll-driven flywheel: four nodes on a circle whose arcs draw in sequence
   as the section scrolls into view. When the loop closes, particles circulate
   continuously — the wheel feeds itself. prefers-reduced-motion renders the
   completed loop with no particles. */
...
const STEPS = [
  ...
  { n: '04', title: 'Intelligence', body: 'The layer that runs voice-operated jobsites — and sharpens the next capture.' },
]

// Desktop corner slots for the four step cards, clockwise from top-right to
// match the node order on the circle.
const CORNERS = [
  'md:top-0 md:right-0',
...
const NODE_DEG = [-45, 45, 135, 225] // clockwise from top-right (SVG y points down)
...
export default function MoatFlywheel() {
  ...
  useEffect(() => {
    ...
    const onScroll = () => {
      ...
      const start = vh * 0.92 // begin drawing as the wheel enters
      const end = vh * 0.45 //   finish by the time it reaches mid-viewport
  ...
  // Arc i (node i → node i+1) draws over its slice of the scroll progress.
  const seg = (i: number) => clamp((p - i * 0.24) / 0.2)
  const nodeOn = (i: number) => (i === 0 ? p > 0.02 : seg(i - 1) > 0.96)
  const done = seg(3) > 0.99

  return (
    <div ref={ref} className="relative mt-8 md:h-[560px]">
      <svg
        viewBox="0 0 440 440"
        aria-hidden="true"
